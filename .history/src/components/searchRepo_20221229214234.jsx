...
const Users = () => {
  ...
  const handleSubmit = (e) => {
    ...
    fetch(`https://api.github.com/users/${inputs.github}/repos`)
      ...
      // SET LOCAL STORAGE
    localStorage.setItem('gitHub', JSON.stringify(inputs.github));
    resetForm();
  };
  ...
  // GET STORAGE
  const getStorage = () => {
    let storage = localStorage.getItem('gitHub');
    if (storage) {
  ...
  return (
    <div className="searchRepo">
      ...
      <form className="searchRepo--form" onSubmit={handleSubmit}>
        ...
        />
        <button type="submit" className="searchRepo--btn">Search</button>
      </form>
          ...
          {currentUsers.map((user) => {
            ...
            return (
              <li key={user.id} className="searchRepo--list">
                ...
                <a href={html_url} target='_blank' rel="noreferrer" className="searchRepo--list__link">
                  Open in Github
                </a>
              </li>
            );
      ...
      <div className="searchRepo--pagination">
