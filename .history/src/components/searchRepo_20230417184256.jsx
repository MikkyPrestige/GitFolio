        ...
        <div className="searchRepo--results">
          <ul className="searchRepo--lists">
            ...
            {currentUsers.map((user, index) => {
              ...
              return (
                // <li key={userindex} className="searchRepo--list">
                <li key={index} className="searchRepo--list">
                  <h3 className="searchRepo--list__title"><strong>Repository Name: </strong>{name}</h3>
                  {description && (
                    <p className="searchRepo--list__content"><strong>Repository Description: </strong>{description}</p>
                  )}
                  {language && (
                    <p className="searchRepo--list__content"><strong>Repository Language: </strong>{language}</p>
                  )}
                  {topics > 0 && (
                    <p className="searchRepo--list__content"><strong>Repository Topics: </strong>{topics}</p>
                  )}
                  {forks > 0 && (
                  <p className="searchRepo--list__content"><strong>Repository Forks: </strong>{forks}</p>
                  )}
                  {/* {license && (
                  <p className="searchRepo--list__content"><strong>Repository License: </strong>{license}</p> */}
                  {/* )} */}
                  {login && (
                  <p className="searchRepo--list__content"><strong>Repository Owner: </strong>{login}</p>
                  )}
                  <a
                    href={html_url}
                    target="_blank"
