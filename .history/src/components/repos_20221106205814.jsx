...
const Repos = () => {
  ...
  return (
    <div className="repos">
      ...
      <div>
        <ol className="repos--items">
        {portfolio.map(repo => {
          return (
            <li key={repo.id} className="repos--list">
              <h3 className="repos--list__item"> {repo.name}</h3>
              <NavLink to={`/repos/${repo.name}`} className="repos--btn">View Repo Data</NavLink>
            </li>
          )})}
        </ol>
      </div>
      <div className="pagination">
        <button onClick={() => setPage(page - 1)} className="repos--btn">Prev</button>
        <button onClick={() => setPage(page + 1)} className="repos--btn">Next</button>
      </div>
      <Outlet />
    </div>
  );
